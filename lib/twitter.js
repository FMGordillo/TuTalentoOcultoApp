...
function Twitter(url) {
  ...
  self.search = function (size, from, callback) {
    request.get(
      /*
      ...
      */
      url + '/api/v1/messages/search?' + qs.stringify({
        count: size,
        q: 'from:' + from,
        result_type: 'recent'
      }), {
  ...
  self.searchTrack = function (trackId, query, size, callback) {
    request.get(
      ...
      // "&from=" + encodeURIComponent(from) +
      "&q=from" + encodeURIComponent(query), {
      // "&q=" + encodeURIComponent(query), {
        json: true
      },
      function (error, response, body) {
  ...
  // self.uploadMedia = function(media, mediaData, additionalOwners) {
  //   request.post(
  //     url + ""
  //   );
  // }
